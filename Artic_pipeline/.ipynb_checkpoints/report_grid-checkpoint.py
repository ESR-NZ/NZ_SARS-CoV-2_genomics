...
def GatherFiles(file_path, file_type="fasta"):
    ...
    #files_paths = [os.path.join(file_path, file) for file in glob.glob(file_path+"*."+file_type)] ## glob was causing something to break?
    
    for fasta_file in files_paths:
        record = SeqIO.read(fasta_file, file_type)
        
        sid = record.id.split('/')[-3].split('_')[-1] ##had to change this for the gridion runs because fasta header is different...
        #print(record.id.split('/')[-3].split('_')[-1])
        if DEBUG:
            print(record.id, sid)
